...
//!
//! With several `bogbogproxd` proxies sharing one Postgres, each captures traffic
//! locally. To let every operator see *everyone's* events live, we relay the
//! local event bus through Postgres `LISTEN/NOTIFY`:
//!
//! - a **publisher** forwards every local event to `NOTIFY bogbogprox_events` (tagged
...
const CHANNEL: &str = "bogbogprox_events";
...
pub fn start(
    ...
    std::thread::Builder::new()
        .name("bogbogprox-pg-notify".into())
        .spawn(move || {
            let mut client = None;
    ...
    // LISTEN thread.
    std::thread::Builder::new()
        .name("bogbogprox-pg-listen".into())
        .spawn(move || loop {
...
fn random_origin() -> u64 {
    ...
    if getrandom::getrandom(&mut buf).is_err() {
        return bogbogprox_core::now_millis() as u64;
    }
