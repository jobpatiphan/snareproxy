...
- โมเดล WIT mirror `snare-core::model` — convert ที่ boundary.

**Decision (ADR W-002):** WIT world `snare:plugin@0.x`; hook = on-request/on-response/passive-scan; host cap = log/add-finding/http-send/var/kv.
...
→ ใช้ **plugin-actor thread** (เหมือน pattern ของ `snare-store-postgres` DB-actor):
...
| เฟส | ทำ | Done เมื่อ |
|---|---|---|
| **P1 · Host + hooks พื้นฐาน** | crate `snare-plugin` (wasmtime + WIT), plugin-actor, on-request/on-response, host `log`, load จาก dir, fuel/epoch limit, 1 ตัวอย่าง (Rust→wasm) | plugin แก้ header ของ request ที่วิ่งผ่านได้จริง |
...
- **ใหม่:** crate `snare-plugin` (wasmtime host, WIT bindings, actor, limits, loader); `wit/snare.wit`; `Plugins` coordinator (list/enable/config); API + Web panel; ตัวอย่าง `examples/plugins/*`.
...
เริ่ม **P1**: crate `snare-plugin` (wasmtime host + `wit/snare.wit` + plugin-actor + fuel/epoch),
